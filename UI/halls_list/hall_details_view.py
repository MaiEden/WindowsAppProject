"""
HallDetailsView: Detail view for a single Hall record.
"""
...
# --- small label factories (tiny helpers to keep UI code terse) -------------
...
# Build a two-column key/value row (used in the meta/contact section)
def _kv(k,v):
...
class HallDetailsView(QWidget):
    ...
    def _build(self):
        """Assemble the static widget/layout structure and lightweight styles."""
        root = QVBoxLayout(self)
        root.setContentsMargins(10,10,10,10)
        root.setSpacing(10)

        # Scroll container keeps the details usable on small windows
        scroll = QScrollArea()
        ...
        # --- Location card (address + map actions) ---------------------------
        ...
        # State fields updated by populate()
        self._current_coords = None       # (lat, lon) tuple or None
        self._current_addr_str = None     # resolved address string (for copy action)
        self._website_url = None          # stored URL (display-only here)
    ...
    def populate(self, r: Dict[str, Any]):
        """
        Render a hall record `r` (keys follow DB naming).
        - Loads the image (with a fallback URL) using the shared async loader.
        - Updates title/subtitle, tags, pricing, and location/contact blocks.
        """
        # Image (async; scaled to the fixed photo size)
        ...
        # Tags (built dynamically based on boolean flags)
        ...
        # Location (server may add Address dict on GET /DB/halls/get/{id})
        ...
        # Enable/disable actions based on whether we have coordinates/address
    ...
    # --- Actions --------------------------------------------------------------
    def _open_maps(self):
        """Open the hall coordinates in the default browser (Google Maps query)."""
    ...
    def _copy_address(self):
        """Copy the resolved address string to the system clipboard (best-effort)."""
        ...
        try:
            ...
            pass  # don't fail the view on copy errors

    # --- Helpers --------------------------------------------------------------
    def _money(self, v) -> str:
        """Format a numeric value as ₪<int>; fallback to '—' when missing."""
        try:
    ...
    def _clear(self, layout):
        """Remove and detach all child widgets from a layout (safe reuse)."""
        while layout.count():
            it = layout.takeAt(0)
            w = it.widget()
