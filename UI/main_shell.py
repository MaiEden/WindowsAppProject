...
class AppWindow(QMainWindow):
    ...
    def goto(self, name: str):
        ...
        if w is not None: self._stack.setCurrentWidget(w)
...
class MainShell(QWidget):
    ...
    def _build_ui(self):
        ...
        if not pm.isNull(): logo.setPixmap(pm.scaledToHeight(56, Qt.SmoothTransformation))

        title = QLabel(f"Welcome, {self.username}"); title.setStyleSheet("font-size:20px;font-weight:700;")
        ...
        def mkbtn(text, emoji):
            ...
            b.setMinimumHeight(44); b.setCursor(Qt.PointingHandCursor); b.setCheckable(True)
            b.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed); return b

        self.btn_events   = mkbtn("Events","📅")
        ...
        self.center_stack = QStackedWidget(); self.center_stack.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        mid.addWidget(side,0); mid.addWidget(self.center_stack,1)
    ...
    def _wire(self):
        ...
        self.btn_events.clicked.connect(lambda: self.navigate("events"))
    ...
    def _placeholder(self, text: str) -> QWidget:
        ...
        lbl = QLabel(text, alignment=Qt.AlignCenter); lbl.setStyleSheet("font-size:16px; color:#666;")
        lay.addWidget(lbl); lay.addStretch(1); return w
    ...
    def go_back(self):
        if self._hist_index > 0:
            self._hist_index -= 1; name = self._history[self._hist_index]
            self.center_stack.setCurrentWidget(self._center_pages[name]); self._update_nav_buttons()
            for key, btn in self._nav_buttons: btn.setChecked(key == name)
